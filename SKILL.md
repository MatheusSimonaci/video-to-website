...
description: "Use this skill whenever the user wants to turn a video file (MP4, MOV, etc.) into a premium scroll-driven animated website. Triggers include: any mention of 'video to website', 'scroll animation', 'canvas frames', 'GSAP scroll', or requests to build an interactive website from a video. Use when the user provides a video file and wants a polished, Apple-style product showcase with frame-by-frame canvas playback driven by scroll position, Lenis smooth scroll, GSAP animations, staggered section reveals, animated counters, horizontal marquee, dark overlay stats, and a circle-wipe hero transition. Do NOT use for general web development, video editing, or tasks unrelated to scroll-driven video websites."
---
